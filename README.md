...
- **Task lists** with real checkboxes; checked state persists, Enter/Backspace behave like Notion. Tab and Shift+Tab nest sub-items to any depth, and the indent survives the trip out to Markdown and back. Converting a bullet list to checkboxes keeps its sub-bullets as sub-checkboxes; converting an indented one back gives you the bullet list again
...
- The site is `index.html`, `privacy.html`, `terms.html`, `manifest.webmanifest`, `sw.js`, `icons/`, `robots.txt`, `sitemap.xml` and `llms.txt`. Everything else in the repo is developer tooling and is not deployed. The install icons in `icons/` are the same artwork as the inline SVG favicon, rasterised once; the throwaway script that produced them is not kept in the tree (it is in the history if the artwork ever changes).
- Crawling is open to everyone, on purpose. `robots.txt` disallows nothing and names the sitemap; the comment in it explains which AI user agents are distribution (`OAI-SearchBot`, `Claude-SearchBot`, `PerplexityBot`, `Bingbot`) and which are only a training opt-out (`GPTBot`, `ClaudeBot`, `Google-Extended`, `Applebot-Extended`), so nobody blocks the wrong half later. `sitemap.xml` lists the same three pages the deploy workflow copies — change one and change the other. `index.html` carries a `SoftwareApplication` JSON-LD block with no `aggregateRating`, because there are no real reviews to put in it.
