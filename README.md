...
- The site is `index.html`, `privacy.html`, `terms.html`, `manifest.webmanifest`, `sw.js` and three PNG icons. The icons are generated by [tools-mkicon.py](tools-mkicon.py) from the same artwork as the inline SVG favicon — pure Python, no image library, so there is nothing to install to regenerate them.
...
That starts a static server, runs [tests.html](tests.html) in headless Chrome
...
You can also just open `tests.html` in a browser — same suite
